...
if(demo){
  ...
  // ---- Market refresh (Phases 30-35) ----
  Promise.allSettled([getJSON('/api/commerce/readiness'),getJSON('/api/analytics/market'),getJSON('/api/security'),getJSON('/api/analytics/ga4'),getJSON('/api/algolia'),getJSON('/api/commerce/channels'),getJSON('/api/webmcp')])
  .then(([commerceRes,marketRes,securityRes,gaRes,algoliaRes,channelsRes,webmcpRes])=>{
    if(commerceRes.status!=='fulfilled')return signedOut('commerce','commerce standards and agent security');
    const c=commerceRes.value,m=marketRes.status==='fulfilled'?marketRes.value:{},s=securityRes.status==='fulfilled'?securityRes.value:{assessments:[]},ga=gaRes.status==='fulfilled'?gaRes.value:{status:'not_connected'},al=algoliaRes.status==='fulfilled'?algoliaRes.value:{},channels=channelsRes.status==='fulfilled'?channelsRes.value:{},web=webmcpRes.status==='fulfilled'?webmcpRes.value:{};
    const feed=c.openAiFeed&&c.openAiFeed.summary||{},ucp=c.ucp||{},latestSecurity=(s.assessments||[])[0]||{},latestPlan=(web.plans||[])[0]||{};
    ...
    set('commerce','<div class="metrics"><div class="metric"><span class="muted">Native UCP</span><b>'+esc(ucp.status||'not checked')+'</b></div>'
      ...
      +'<div class="formRow"><button id="probeUcp" class="btn">Check native UCP</button><a class="btn" href="/api/commerce/openai-feed.jsonl">Preview OpenAI feed</a></div><label class="muted"><input id="ucpDev" type="checkbox"> I confirm this is an authorised development store</label><button id="negotiateUcp" class="btn" style="margin-top:8px">Run safe UCP negotiation</button><div id="ucpResult" class="muted" style="margin-top:10px"></div></div>'
      ...
      +(ga.status==='connected'?'<div class="chip">Connected · property '+esc(ga.propertyId||'not selected')+'</div><div class="formRow"><input id="gaProperty" class="input" inputmode="numeric" placeholder="GA4 property ID"><button id="saveGaProperty" class="btn">Save property</button></div><div class="formRow"><input id="gaStart" class="input" type="date"><input id="gaEnd" class="input" type="date"><button id="importGa" class="btn">Import GA4</button></div><div id="gaResult" class="muted"></div>':'<a class="btn" href="/api/analytics/ga4/connect">Connect GA4</a>')+'</div></div>'
      ...
      +'<div class="two"><div class="card"><h3>Lighthouse Agentic Browsing</h3><input id="lighthouseFile" class="input" type="file" accept="application/json"><button id="importLighthouse" class="btn" style="margin-top:8px">Import report</button><div id="lighthouseResult" class="muted"></div></div>'
      +'<div class="card"><h3>Algolia (optional)</h3><p class="muted">'+esc(al.note||'Not checked')+'</p><div class="formRow"><input id="algoliaApp" class="input" placeholder="Application ID"><input id="algoliaIndex" class="input" placeholder="Index name"></div><div class="formRow"><input id="algoliaKey" class="input" type="password" autocomplete="off" placeholder="Restricted Search-only key"><input id="algoliaQuery" class="input" placeholder="Buyer-intent query"><button id="auditAlgolia" class="btn">Run one-time audit</button></div><div id="algoliaResult" class="muted"></div></div></div>'
      +'<div class="two"><div class="card"><h3>Action security</h3><p class="muted">A display label is self-declared, not verified identity. Unannotated tools are treated as consequential.</p><input id="securityLabel" class="input" placeholder="Target label"><textarea id="securityTools" class="input" rows="5" placeholder="Paste the discovered tools JSON array"></textarea><button id="assessSecurity" class="btn">Assess tools</button><div id="securityResult" class="muted"></div></div>'
      +'<div class="card"><h3>Fix My Site actions</h3><p class="muted">'+esc(latestPlan.status||'No plan yet')+' · '+esc(latestPlan.implementation_route||'No implementation route selected')+'</p><button id="buildWebPlan" class="btn">Build safe action plan</button><div id="webPlanResult" class="muted"></div><p class="muted" style="font-size:12px">Planning and tests are not a live installation. WordPress starts disabled and every route remains reversible.</p></div></div>'
      +'<div class="card" style="margin-top:16px"><h3>Native AI-channel evidence</h3><p class="muted">Merchant Center imports: '+num((channels.merchantAiPerformance||[]).length)+' · Shopify channel observations: '+num((channels.shopifyChannels||[]).length)+' · conversational product readiness: '+num(channels.conversationalProducts&&channels.conversationalProducts.ready)+' / '+num(channels.conversationalProducts&&channels.conversationalProducts.total)+'. Imported evidence is never called Live.</p>'
      +'<div class="two"><div><h4>Merchant Center organic export</h4><div class="formRow"><input id="mcAccount" class="input" placeholder="Merchant account ID"><input id="mcCategory" class="input" placeholder="Category"><input id="mcCountry" class="input" placeholder="Country code"><input id="mcLanguage" class="input" placeholder="Language"></div><div class="formRow"><input id="mcStart" class="input" type="date"><input id="mcEnd" class="input" type="date"></div><textarea id="mcMetrics" class="input" rows="4" placeholder="Scoped metrics JSON"></textarea><label class="muted"><input id="mcAuth" type="checkbox"> I confirm this authorised export is organic-only</label><button id="importMc" class="btn">Import scoped evidence</button><div id="mcResult" class="muted"></div></div>'
      +'<div><h4>Shopify agentic channel</h4><select id="shopifyChannel" class="input"><option value="chatgpt">ChatGPT</option><option value="google">Google</option><option value="microsoft_copilot">Microsoft Copilot</option><option value="meta">Meta</option></select><label class="muted"><input id="channelDiscovery" type="checkbox"> Discovery enabled</label><label class="muted"><input id="channelCheckout" type="checkbox"> Direct checkout enabled</label><label class="muted"><input id="channelAuth" type="checkbox"> I observed this in authorised Shopify Admin</label><button id="saveChannel" class="btn">Record channel state</button><div id="channelResult" class="muted"></div></div></div></div>');
    ...
    document.querySelector('#negotiateUcp').onclick=()=>{const out=document.querySelector('#ucpResult');if(!document.querySelector('#ucpDev').checked){out.textContent='Confirm the development-store restriction first.';return;}getJSON('/api/commerce/ucp/negotiate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({optIn:true,developmentStore:true})}).then(r=>{out.textContent=(r.run&&r.run.status||'complete')+' · no cart, checkout or order is permitted';}).catch(e=>{out.textContent=e.message;});};
    if(document.querySelector('#saveGaProperty'))document.querySelector('#saveGaProperty').onclick=()=>getJSON('/api/analytics/ga4/property',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({propertyId:document.querySelector('#gaProperty').value})}).then(()=>{document.querySelector('#gaResult').textContent='Property saved.';}).catch(e=>{document.querySelector('#gaResult').textContent=e.message;});
    if(document.querySelector('#importGa'))document.querySelector('#importGa').onclick=()=>getJSON('/api/analytics/ga4/import',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({startDate:document.querySelector('#gaStart').value,endDate:document.querySelector('#gaEnd').value})}).then(r=>{document.querySelector('#gaResult').textContent='Imported '+num(r.rows)+' authorised GA4 row(s).';}).catch(e=>{document.querySelector('#gaResult').textContent=e.message;});
    document.querySelector('#importLighthouse').onclick=async()=>{const file=document.querySelector('#lighthouseFile').files[0],out=document.querySelector('#lighthouseResult');if(!file){out.textContent='Choose a Lighthouse JSON report.';return;}try{const report=JSON.parse(await file.text()),r=await getJSON('/api/commerce/lighthouse/import',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(report)});out.textContent='Imported '+num(r.passed)+' passed and '+num(r.failed)+' failed audit(s).';}catch(e){out.textContent=e.message;}};
    document.querySelector('#auditAlgolia').onclick=()=>{const out=document.querySelector('#algoliaResult');getJSON('/api/algolia/audit',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({applicationId:document.querySelector('#algoliaApp').value,indexName:document.querySelector('#algoliaIndex').value,searchKey:document.querySelector('#algoliaKey').value,query:document.querySelector('#algoliaQuery').value})}).then(r=>{document.querySelector('#algoliaKey').value='';out.textContent=r.status+' · '+num(r.hitCount)+' hit(s); key was not stored.';}).catch(e=>{document.querySelector('#algoliaKey').value='';out.textContent=e.message;});};
    document.querySelector('#assessSecurity').onclick=()=>{const out=document.querySelector('#securityResult');try{const tools=JSON.parse(document.querySelector('#securityTools').value);getJSON('/api/security/assess',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({targetLabel:document.querySelector('#securityLabel').value,tools})}).then(r=>{out.textContent=r.status+' · '+num(r.mutatingToolCount)+' consequential tool(s); identity remains unverified unless independently evidenced.';}).catch(e=>{out.textContent=e.message;});}catch{out.textContent='Tools must be a valid JSON array.';}};
    document.querySelector('#buildWebPlan').onclick=()=>getJSON('/api/webmcp/plan',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({signals:{wordpress:false}})}).then(r=>{document.querySelector('#webPlanResult').textContent=r.status+' · '+r.route.route+' · '+(r.actions||[]).filter(a=>a.applicable).length+' applicable action(s).';}).catch(e=>{document.querySelector('#webPlanResult').textContent=e.message;});
    document.querySelector('#importMc').onclick=()=>{const out=document.querySelector('#mcResult');let metrics={};try{metrics=JSON.parse(document.querySelector('#mcMetrics').value||'{}');}catch{out.textContent='Metrics must be valid JSON.';return;}getJSON('/api/commerce/merchant-ai/import',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({authorized:document.querySelector('#mcAuth').checked,organicOnly:document.querySelector('#mcAuth').checked,merchantAccountId:document.querySelector('#mcAccount').value,category:document.querySelector('#mcCategory').value,country:document.querySelector('#mcCountry').value,language:document.querySelector('#mcLanguage').value,windowStart:document.querySelector('#mcStart').value,windowEnd:document.querySelector('#mcEnd').value,metrics})}).then(r=>{out.textContent='Imported '+r.evidenceTier+' for '+r.scope.country+' / '+r.scope.category+'.';}).catch(e=>{out.textContent=e.message;});};
    document.querySelector('#saveChannel').onclick=()=>{const out=document.querySelector('#channelResult');getJSON('/api/commerce/shopify-channels/import',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({authorized:document.querySelector('#channelAuth').checked,channel:document.querySelector('#shopifyChannel').value,discoveryEnabled:document.querySelector('#channelDiscovery').checked,directCheckoutEnabled:document.querySelector('#channelCheckout').checked})}).then(r=>{out.textContent='Recorded '+r.channel+' · attribution '+r.attributionSource+'.';}).catch(e=>{out.textContent=e.message;});};
  });
