...
export async function getActions(env:Env,shop:string):Promise<AiAction[]>{
  ...
  return [
    ...
    {type:"browse_catalog",supported:hasCatalog,url:hasCatalog?`${site}/collections/all`:undefined,
     ...
     limitations:hasCatalog?undefined:"No catalogue has been synced for this business yet."},
    {type:"checkout_handoff",supported:hasCatalog,url:hasCatalog?String(items[0].url||site):undefined,
     description:"Open a merchant-owned product page so the customer can choose to continue through the merchant's cart and checkout.",
     source:"Merchant catalogue synced from Shopify.",
     limitations:hasCatalog?"AgentCart does not add an item, submit checkout or purchase. The customer continues on the merchant's site.":"No catalogue has been synced for this business yet."},
    ...
    {type:"request_quote",supported:!!email,url:email?`mailto:${email}`:undefined,
     description:"Hand the customer to the business's published email address to request a quote.",
     source:"Merchant's published contact address.",
     limitations:email?"AgentCart does not send the enquiry or include customer data.":"The business has not published a contact email."},
