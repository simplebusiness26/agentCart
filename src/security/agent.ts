...
export interface AgentSecurityFinding {key:"untrusted_metadata"|"untrusted_output"|"identity"|"scope"|"approval"|"idempotency"|"rollback"|"verification";state:SecurityState;detail:string;tools:string[];}
...
const MUTATION=/^(create|update|delete|remove|send|pay|purchase|buy|book|cancel|refund|publish|execute|run|set|add|submit|place)_/i;
const INJECTION=[
...
export function assessAgentInteractionSecurity(input:{tools:SecurityTool[];outputText?:string;identity?:string}){
  const tools=(input.tools||[]).slice(0,500),names=tools.map(t=>text(t.name,128)).filter(Boolean),mutating=tools.filter(t=>MUTATION.test(text(t.name))||t.annotations?.readOnlyHint===false);
  ...
  const findings:AgentSecurityFinding[]=[
    ...
    {key:"identity",state:input.identity?"pass":"unknown",tools:names,detail:input.identity?`The tool server identifies as ${text(input.identity,160)}.`:"No independently verified tool-server identity was supplied."},
  ...
  const status=metadataRisks.length||outputRisk||findings.some(f=>f.state==="fail")?"fail":findings.some(f=>f.state==="unknown")?"unknown":"pass";
